...
#' @param add_nontarget TRUE/FALSE to add residuals and target component spectra to the output. Only applies if output_mat = FALSE.
#' @param bounds a nested list generated by the parent function that specifies the component boundaries, used for constrained wavelength comparisons. Will restrict the sample spectra to the wavelengths specified by said mechanism.
#'
...
comp_correct_spectra <- function(grob = NULL, sample_char, comp, type = "ex",
  ...
  if(is.null(grob)){
    stop("Please supply a grob with which to correct spectra.")
  }
  ...
  if(isTRUE(output_mat)){
    ...
    if(!is.null(bounds)){
      # Interpolate.
      mat <- interpolate_matrix_1nm(mat)
      # Trimming spectra in line with component threshold bounds set in this or the parent function.
      if(length(unique(corrected_spectra$em)) == 1){
        # type = "ex". Ex spectra; use associated vars
        min_ex <- eemUtils::binary_search_nearest(mat$wavelength,value = as.numeric(unlist(bounds[[1]][[1]][1])))
        max_ex <- eemUtils::binary_search_nearest(mat$wavelength,value = as.numeric(unlist(bounds[[1]][[1]][2])))
        mat <- mat %>%
          slice(which(mat$wavelength == min_ex):which(mat$wavelength == max_ex)) %>% column_to_rownames('wavelength') %>%
          data.matrix()
      } else {
        type = "em"
        min_em <- eemUtils::binary_search_nearest(mat$wavelength,value = as.numeric(unlist(bounds[[2]][[1]][1])))
        max_em <- eemUtils::binary_search_nearest(mat$wavelength,value = as.numeric(unlist(bounds[[2]][[1]][2])))
        mat <- mat %>%
          slice(which(mat$wavelength == min_em):which(mat$wavelength == max_em)) %>% column_to_rownames('wavelength') %>%
          data.matrix()
      }
    }
    mat
